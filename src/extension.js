        ...
        vscode.languages.registerSignatureHelpProvider(
            'markdown',
            {
                provideSignatureHelp: (document, position) => {
                        ...
                        if (state.text[start - 1] === '$' && state.text[start] !== ' ') {
                            while (++end < state.text.length) {
                                if (state.text[end] === '$' && state.text[end - 1] !== ' ') {
                                    if (start < end)
                                        latexElement = `![latexPreview](${svgToUri(
                                            texToSvg(state.text.slice(start, end))
                                        )})`;
                                    break;
                                }
                            }
                    ...
                    const text = document
                        .lineAt(cursorPosition)
                        .text.replace(new RegExp(`(?<=^.{${position.character}})`), '█');
...
function registerWebviewViewProvider(context) {
    ...
    webviewLoaded = new Promise((resolve) => {
        ...
        vscode.window.registerWebviewViewProvider(
            'test.webview',
            {
                resolveWebviewView: (webviewView) => {
                    ...
                    const mermaidScriptUri = webviewView.webview.asWebviewUri(
        ...
        [
            [
                ...
                [
                    'heading',
                    (() => {
                        const getEnlargeDecoration = memoize((size) =>
                        ...
                        return (start, end, node) => {
                            ...
                            addDecoration(
                                getEnlargeDecoration((5 * state.fontSize) / (2 + node.depth)),
                                start + node.depth + 1,
                                end
                            );
                    ...
                    (() => {
                        const horizontalRuleDecoration = vscode.window.createTextEditorDecorationType({
                            ...
                            before: {
                                contentText: '',
                                textDecoration:
                                    'none; position: absolute; background: #ffaa00; top: 0.49em; bottom: 0.49em; width: 100%; mix-blend-mode: luminosity; border: outset;',
                            },
                        });
                        return (start, end) => {
                    ...
                    (() => {
                        ...
                        const quoteBarDecoration = vscode.window.createTextEditorDecorationType({
                            ...
                            before: {
                                contentText: '',
                                textDecoration:
                                    'none; position: absolute; background: #ffaa00; top: -0.2em; bottom: -0.2em; width: 3px; border-radius: 99px; mix-blend-mode: luminosity;',
                            },
                        });
                        return (start, end) => {
                ...
                [
                    ...
                    (() => {
                        ...
                        const getlistRainbowDecoration = (() => {
                            ...
                            const getNonCyclicDecoration = memoize((level) =>
                                vscode.window.createTextEditorDecorationType({
                                    textDecoration: `; filter: hue-rotate(${
                                        (hueRotationMultiplier[level] * 360) / 12
                                    }deg);`,
                                })
                            );
                            return (level) => {
                ...
                [
                    ...
                    (() => {
                        const getTexDecoration = (() => {
                            ...
                            return (texString, display, numLines) =>
                                _getTexDecoration(
                                    texString,
                                    display,
                                    state.darkMode,
                                    state.fontSize,
                                    numLines * state.lineHeight
                                );
                        })();
                        return (start, end) => {
                    ...
                    (() => {
                        const getMermaidDecoration = (() => {
                            const _getTexDecoration = memoize(async (source, darkMode, height, fontFamily) => {
                                await webviewLoaded;
                                const svgString = await requestSvg({
                                    source: source,
                                    darkMode: darkMode,
                                ...
                                const maxWidth =
                                    (parseFloat(svgNode.css('max-width')) * height) /
                                    parseFloat(svgNode.attr('height'));
                                const svg = svgNode
                            ...
                            return (source, numLines) =>
                                _getTexDecoration(
                                    source,
                                    state.darkMode,
                                    (numLines + 2) * state.lineHeight,
                                    state.fontFamily
                                );
                        })();
            ...
            [
				...
				`;
                            const temp = html.match(/<tr>[^]+?<\/tr>/g).map((r) =>
                                r
                                    .replace(/^<tr>\n<t[dh]>/, '')
                                    .split(/<t[dh]>/)
                                    .map((c) => c.replace(/<\/?("[^"]*"|'[^']*'|[^>])*(>|$)/g, ''))
                            );
                            const maxLength = temp
                                .reduce(
                                    (acc, cur) => acc.map((val, idx) => Math.max(val, cur[idx].length)),
                                    Array(temp[0].length).fill(0)
                                )
...
function activate(context) {
    ...
    vscode.window.onDidChangeTextEditorVisibleRanges(
        (event) => {
            ...
            if (
                state.activeEditor &&
                state.activeEditor.document.lineCount > 500 &&
                event.textEditor.document === state.activeEditor.document
            ) {
                triggerUpdateDecorations();
            }
        },
